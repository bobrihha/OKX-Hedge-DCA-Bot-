...
class HedgeTradingBot:
    ...
    def _start_new_cycle(self, side):
        ...
        logger.info(f"[{side.upper()}] Расчетный размер первого ордера: {order_size:.4f} (маржа: ${margin:.2f})")

        order_side = 'buy' if side == 'long' else 'sell'
        result = okx_api.place_order(instId=config.TRADING_PAIR, side=order_side, ordType='limit', sz=order_size, px=current_price, posSide=side)
    ...
    def _check_orders_status(self, side):
        ...
        for order_id in list(position.pending_orders.keys()):
            ...
            if details and details.get('data') and details['data'][0].get('state') == 'filled':
    ...
    def _handle_filled_order(self, side, details):
        ...
        for attempt in range(1, 4):
            logger.info(f"[{side.upper()}] Размещение TP ордера по цене {tp_price:.4f} (Попытка {attempt}/3)")
            tp_res = okx_api.place_order(instId=config.TRADING_PAIR, side=tp_side, sz=position.total_position_qty, px=tp_price, posSide=side, ordType='limit')
        ...
        if position.executed_orders_count < config.MAX_ORDERS_IN_CYCLE:
            ...
            next_res = okx_api.place_order(instId=config.TRADING_PAIR, side=next_side, sz=next_size, px=next_price, posSide=side, ordType='limit')
